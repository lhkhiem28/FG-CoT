...
class BaselineLLM(torch.nn.Module):
    ...
    def inference(self, samples):
        ...
        generations = None
        if self.num_return_sequences > 1 and not self.accuracy_only:
            with self.maybe_autocast():
                outputs = self.model.generate(
                    inputs_embeds=inputs_embeds,
                    attention_mask=attention_mask,
                    pad_token_id=self.tokenizer.eos_token_id,
                    max_new_tokens=self.max_completion_length,
                    do_sample=True, temperature=self.temperature, top_k=50, top_p=1.0,
                    num_return_sequences=self.num_return_sequences,
                    use_cache=True  # IMPORTANT!
                )
            gens = self.tokenizer.batch_decode(outputs, skip_special_tokens=True)
            gens = [g.strip() for g in gens]
            # generate interleaves the samples, so rows [i*k, (i+1)*k) belong to batch item i
            generations = ["|".join(gens[i*self.num_return_sequences:(i+1)*self.num_return_sequences]) for i in range(batch_size)]

        outputs = {"id": samples["id"],
                   "label": samples["smiles"],
                   "pred": preds,
                   "codes": samples["codes"], "texts": samples["texts"]
        }
